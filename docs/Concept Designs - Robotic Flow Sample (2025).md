...
* [LiteState (C#) - MK2 States with Messages](#litestate-c---mk2-states-with-messages)
  * [Context Properties and Methods](#context-properties-and-methods)
    * [Context - `SendMessage`](#context---sendmessage)
  * [Simple State Transitioning](#simple-state-transitioning)
  * [Forced-State Transition](#forced-state-transition)
  * [Last State Transition History](#last-state-transition-history)
  * [Example State machine](#example-state-machine)
    * [State Tree](#state-tree)
    * [Generic Timeout Handler](#generic-timeout-handler)
    * [Sample](#sample)
...
## Last State Transition History

The `context` property `LastState` can be used to tell you which state we came from. This can be helpful when you have multiple entrypoints such as an error or faulted state transition.

```cs
public void Faulted_OnEnter(Context ctx)
{
  // Which state faulted.
  var lastState = ctx.LastState;
  var errorParams = ctx.Params(ParameterType.Errors);
  Console.WriteLine($"The state {lastState} faulted.");
}
```
